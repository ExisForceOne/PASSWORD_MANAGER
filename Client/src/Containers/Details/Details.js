...
import {
  ...
  Url,
  Desc,
} from "../../Components/Details/Section/Section";
...
export default function Details(props) {
  ...
  return (
    <div>
      <Header color={data.color} name={data.name} fav={data.fav}>
      ...
      <Password value={passwordIsVisible ? data.password : "********"}>
        <VisibleBtn state={{ passwordIsVisible, setPasswordIsVisible }} />
        <CopyBtn value={data.password} />
      </Password>

      {data.url ? <Url value={data.url} /> : null}

      {data.desc ? <Desc value={data.desc} /> : null}
    </div>
  );
}
